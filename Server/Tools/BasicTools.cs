...
namespace AnimeShopMcp.Tools;

[McpServerToolType]
public static class BasicTools
{
    [McpServerTool(Name = "ping"), Description("Check that the game is running with AnimeShopBridge loaded, and list the bridge's commands.")]
    ...
    {
        if (!paths.Known)
            return Relay.Error("GAME_PATH is not set, so the log cannot be found. Set it in your Claude Code " +
                               "settings under \"env\", pointing at the folder holding the game's exe.");
