...
namespace AnimeShopBridge.Net
{
    ...
    public sealed class LineServer
    {
        ...
        public void Start()
        {
            ...
            _running = true;
            _acceptThread = new Thread(AcceptLoop) { IsBackground = true, Name = "AnimeShopBridge accept" };
            _acceptThread.Start();
        }
        ...
        private void AcceptLoop()
        {
            while (_running)
            {
                ...
                var thread = new Thread(() => Serve(client)) { IsBackground = true, Name = "AnimeShopBridge client" };
                thread.Start();
            }
        }
        ...
        private void Serve(TcpClient client)
        {
            using (client)
            {
                ...
                {
                    LogError($"[AnimeShopBridge] Connection failed: {ex}");
                }
            }
        }
